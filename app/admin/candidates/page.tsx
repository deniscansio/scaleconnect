      ...
      <div className="flex">
        <aside className="w-72 bg-gray-950 shadow-lg min-h-screen p-6 border-r border-gray-700">
          <nav className="space-y-4">
            <Link href="/admin/dashboard" className="block px-4 py-3 text-gray-300 hover:bg-gray-800 rounded-lg">📊 Dashboard</Link>
            <Link href="/admin/companies" className="block px-4 py-3 text-gray-300 hover:bg-gray-800 rounded-lg">🏢 Gerenciar Empresas</Link>
            <Link href="/admin/candidates" className="block px-4 py-3 bg-yellow-400 text-black rounded-lg font-semibold">👥 Gerenciar Candidatos</Link>
            <Link href="/admin/partners" className="block px-4 py-3 text-gray-300 hover:bg-gray-800 rounded-lg">🎓 Gerenciar Parceiros</Link>
            <Link href="/admin/subscriptions" className="block px-4 py-3 text-gray-300 hover:bg-gray-800 rounded-lg">💳 Assinaturas</Link>
            <Link href="/admin/payments" className="block px-4 py-3 text-gray-300 hover:bg-gray-800 rounded-lg">💰 Pagamentos</Link>
            <Link href="/admin/analytics" className="block px-4 py-3 text-gray-300 hover:bg-gray-800 rounded-lg">📈 Analytics</Link>
            <Link href="/admin/settings" className="block px-4 py-3 text-gray-300 hover:bg-gray-800 rounded-lg">⚙️ Configurações</Link>
          </nav>
        </aside>
        ...
        <div className="flex-1 p-8">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
              {loading ? (
                <div className="p-12 text-center text-gray-400">Buscando dados no TiDB...</div>
              ) : (
                <table className="w-full">
                  ...
                  <tbody>
                    {filteredCandidates.length === 0 ? (
                      <tr><td colSpan={7} className="px-6 py-8 text-center text-gray-500">Nenhum candidato encontrado.</td></tr>
                    ) : (
                        ...
                        <tr key={candidate.id} className="border-b border-gray-700 hover:bg-gray-700 transition">
                          ...
                          <td className="px-6 py-4">
                            <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                              ...
                              candidate.status === 'Bloqueado' ? 'bg-red-500 text-white' : 'bg-yellow-500 text-black'
                            }`}>{candidate.status}</span>
                          </td>
                          <td className="px-6 py-4 text-gray-300">{candidate.level || 'N/A'}</td>
                          ...
                          <td className="px-6 py-4 flex gap-2">
                            {candidate.status === 'Inativo' && (
                              <button onClick={() => handleActivateCandidate(candidate.id)} className="px-3 py-1 bg-green-600 text-white rounded text-xs hover:bg-green-700">Ativar</button>
                            )}
                            {candidate.status === 'Ativo' && (
                              <button onClick={() => handleBlockCandidate(candidate.id)} className="px-3 py-1 bg-red-600 text-white rounded text-xs hover:bg-red-700">Bloquear</button>
                            )}
                            <button className="px-3 py-1 bg-blue-600 text-white rounded text-xs hover:bg-blue-700">Ver Perfil</button>
                          </td>
                        </tr>
                      ))
                    )}
